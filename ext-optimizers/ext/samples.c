...
zend_class_entry *samples_math_ce;

ZEND_DECLARE_MODULE_GLOBALS(samples)
...
static PHP_MINIT_FUNCTION(samples)
{
#if PHP_VERSION_ID < 50500
...
#ifndef ZEPHIR_RELEASE
static PHP_MSHUTDOWN_FUNCTION(samples)
{
 ...
 */
static void php_zephir_init_globals(zend_samples_globals *zephir_globals TSRMLS_DC)
{
...
static PHP_RINIT_FUNCTION(samples)
{

	zend_samples_globals *zephir_globals_ptr = ZEPHIR_VGLOBAL;
...
static PHP_RSHUTDOWN_FUNCTION(samples)
{
...
static PHP_MINFO_FUNCTION(samples)
{
	php_info_print_box_start(0);
	php_printf("%s", PHP_SAMPLES_DESCRIPTION);
	php_info_print_box_end();

	php_info_print_table_start();
	php_info_print_table_header(2, PHP_SAMPLES_NAME, "enabled");
	php_info_print_table_row(2, "Author", PHP_SAMPLES_AUTHOR);
	php_info_print_table_row(2, "Version", PHP_SAMPLES_VERSION);
	php_info_print_table_row(2, "Powered by Zephir", "Version " PHP_SAMPLES_ZEPVERSION);
	php_info_print_table_end();


}

static PHP_GINIT_FUNCTION(samples)
{
	php_zephir_init_globals(samples_globals TSRMLS_CC);
}

static PHP_GSHUTDOWN_FUNCTION(samples)
{

}

zend_module_entry samples_module_entry = {
	...
	PHP_MINIT(samples),
#ifndef ZEPHIR_RELEASE
	PHP_MSHUTDOWN(samples),
#else
	NULL,
#endif
	PHP_RINIT(samples),
	PHP_RSHUTDOWN(samples),
	PHP_MINFO(samples),
	PHP_SAMPLES_VERSION,
	ZEND_MODULE_GLOBALS(samples),
	PHP_GINIT(samples),
	PHP_GSHUTDOWN(samples),
	NULL,
	STANDARD_MODULE_PROPERTIES_EX
};

#ifdef COMPILE_DL_SAMPLES
ZEND_GET_MODULE(samples)
#endif
